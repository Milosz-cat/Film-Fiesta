...
class TMDBClient:
    ...
    It uses the requests library to make HTTP requests and retrieves and process data in
    JSON format.
    ...
    def get_single_movie(self, search_term, year):
        ...
        hours = movie_detalis["runtime"] // 60
        minutes = movie_detalis["runtime"] % 60
        ...
        directors = [
            person for person in movie_persons["crew"] if person["job"] == "Director"
        ]
        writers = [
            person
            for person in movie_persons["crew"]
            if person["job"] in ["Writer", "Screenplay", "Novel"]
        ]
    ...
    def get_movies_by_person(self, person_id):
        ...
        actor_movies = sorted(
            actor_movies, key=lambda x: x.get("popularity", 0), reverse=True
        )
        director_movies = sorted(
            director_movies, key=lambda x: x.get("popularity", 0), reverse=True
        )
